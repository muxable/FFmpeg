...
static int nvmpi_encode_frame(AVCodecContext *avctx, AVPacket *pkt,const AVFrame *frame, int *got_packet){
	...
	if(frame){
		...
		_nvframe.timestamp=av_rescale_q(frame->pts, avctx->time_base, (AVRational){1, 1000000});

		res=nvmpi_encoder_put_frame(nvmpi_context->ctx,&_nvframe);

		if(res<0) {
			av_log(avctx, AV_LOG_WARNING, "failed to put frame %d\n", res);
			return res;
		}
	}
	...
	pkt->dts=pkt->pts=av_rescale_q(packet.pts, (AVRational){1, 1000000}, avctx->time_base);
